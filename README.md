...
2. 打开工具 <kbd>配置</kbd> → <kbd>同步</kbd>：设置请求地址并登录（可退出登录）
3. 主界面工具面板推荐先点 <kbd>对比差异后同步</kbd>：查看仅本地 / 仅云端 / 内容不同，再决定方向
4. 同步方向说明
   1. <kbd>本地 → 云</kbd>：用本地词库覆盖线上备份
   2. <kbd>本地 ← 云</kbd>：用线上备份覆盖本地（需再点保存写入文件）
   3. <kbd>本地 + 云</kbd>：把云端独有词条并入本地后上传
